...
def get_filename_md5_table(file_dir):
    filename_md5_table = {}
    file_paths = [os.path.join(file_dir, filename) for filename in os.listdir(file_dir) if len(filename) != 36]
...
def rename_files_md5(file_dir, filename_md5_table):
    for i, row in tqdm(filename_md5_table.iterrows(), total=len(filename_md5_table), desc='Renaming files'):
